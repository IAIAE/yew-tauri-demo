 ...
 */
async function eventOnce<T>(
    event: EventName,
    windowLabel: string | null,
    handler: EventCallback<T>
): Promise<UnlistenFn> {
    return eventListen<T>(event, windowLabel, (eventData) => {
        handler(eventData)
